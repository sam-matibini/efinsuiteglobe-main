...
import {
  ...
  getPurchaseAttachmentSignedUrl,
...
export function PurchaseAttachmentsSection({
  ...
  const { data: attachments = [], isLoading } = usePurchaseAttachments(entityType, entityId);
  const upload = useUploadPurchaseAttachment();
  const del = useDeletePurchaseAttachment();

  if (!entityId) {
    return (
      <div className="rounded-md border border-dashed p-4 text-sm text-muted-foreground">
        <Paperclip className="inline h-4 w-4 mr-2" />
        {saveHint || 'Save this record first to attach supporting documents.'}
      </div>
    );
  }
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      await upload.mutateAsync({ entityType, entityId, organizationId, file });
    }
    if (inputRef.current) inputRef.current.value = '';
  };

  const open = async (att: PurchaseAttachment) => {
    try {
      const url = await getPurchaseAttachmentSignedUrl(att.file_path);
  ...
  const download = async (att: PurchaseAttachment) => {
    try {
      const url = await getPurchaseAttachmentSignedUrl(att.file_path);
