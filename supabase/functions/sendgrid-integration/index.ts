import { serve } from "https://deno.land/std@0.190.0/http/server.ts";
import { encode } from "https://deno.land/std@0.190.0/encoding/base64.ts";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-supabase-client-platform, x-supabase-client-platform-version, x-supabase-client-runtime, x-supabase-client-runtime-version",
};

// Brand configuration (overridable via secrets)
const BRAND_NAME = Deno.env.get("SENDGRID_FROM_NAME") || "efinsuite Globe";
const BRAND_EMAIL = Deno.env.get("SENDGRID_FROM_EMAIL") || "info@efinsuite.com";

interface SendGridRequest {
  action: "health-check" | "test" | "send-email" | "send-test";

  to?: string;
  subject?: string;
  message?: string;
  html?: string;
  includeBranding?: boolean;
  // Optional single attachment (fetched server-side from a public URL) - legacy
  attachmentUrl?: string;
  attachmentFilename?: string;
  attachmentMimeType?: string;
  // Multiple attachments support (url-based or inline base64)
  attachments?: Array<{
    url?: string;
    content?: string; // base64 encoded content (alternative to url)
    filename?: string;
    mimeType?: string;
  }>;
  // Branding data
  branding?: {
    logoUrl?: string;
    displayName?: string;
    signatureHtml?: string;
    phone?: string;
    email?: string;
    website?: string;
    address?: string;
  };
}

const MAX_ATTACHMENT_BYTES = 20 * 1024 * 1024; // 20MB (safe-ish ceiling for email delivery)

function sanitizeFilename(filename: string): string {
  return filename
    .trim()
    .replace(/[\\/:*?"<>|]+/g, "-")
    .replace(/\s+/g, " ")
    .slice(0, 180);
}

function filenameFromUrl(url: string): string | null {
  try {
    const u = new URL(url);
    const parts = u.pathname.split("/").filter(Boolean);
    const last = parts[parts.length - 1];
    return last ? sanitizeFilename(last) : null;
  } catch {
    return null;
  }
}

interface BrandingData {
  logoUrl?: string;
  displayName?: string;
  signatureHtml?: string;
  phone?: string;
  email?: string;
  website?: string;
  address?: string;
}

function getBrandedHeader(branding?: BrandingData): string {
  if (!branding?.logoUrl && !branding?.displayName) {
    return '';
  }
  
  return `
    <div style="margin-bottom: 24px; padding-bottom: 16px; border-bottom: 1px solid #e5e7eb;">
      ${branding.logoUrl ? `<img src="${branding.logoUrl}" alt="${branding.displayName || BRAND_NAME}" style="max-height: 50px; max-width: 200px; margin-bottom: 8px;">` : ''}
      ${branding.displayName ? `<p style="margin: 0; font-weight: 600; color: #1f2937;">${branding.displayName}</p>` : ''}
    </div>
  `;
}

function getBrandedSignature(branding?: BrandingData): string {
  if (branding?.signatureHtml) {
    return `<div style="margin-top: 24px;">${branding.signatureHtml}</div>`;
  }
  
  // Build a signature from branding data if no custom HTML
  const parts: string[] = [];
  
  if (branding?.displayName) {
    parts.push(`<strong>${branding.displayName}</strong>`);
  }
  if (branding?.phone) {
    parts.push(`📞 ${branding.phone}`);
  }
  if (branding?.email) {
    parts.push(`✉️ <a href="mailto:${branding.email}" style="color: #2563eb;">${branding.email}</a>`);
  }
  if (branding?.website) {
    parts.push(`🌐 <a href="${branding.website}" style="color: #2563eb;">${branding.website}</a>`);
  }
  if (branding?.address) {
    parts.push(`📍 ${branding.address}`);
  }
  
  if (parts.length === 0) {
    return '';
  }
  
  return `
    <div style="margin-top: 24px; padding-top: 16px; border-top: 1px solid #e5e7eb; font-size: 13px; color: #4b5563;">
      ${parts.join('<br>')}
    </div>
  `;
}

function getBrandedFooter(branding?: BrandingData): string {
  const name = branding?.displayName || BRAND_NAME;
  return `
    <div style="margin-top: 32px; padding-top: 16px; border-top: 1px solid #e5e7eb; color: #6b7280; font-size: 12px; text-align: center;">
      <p style="margin: 0;">Sent by ${name}</p>
      <p style="margin: 4px 0 0 0;">© ${new Date().getFullYear()} ${name}. All rights reserved.</p>
    </div>
  `;
}

function wrapWithBranding(html: string, branding?: BrandingData): string {
  return `
    <!DOCTYPE html>
    <html>
      <head>
        <meta charset="utf-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
      </head>
      <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #1f2937; max-width: 600px; margin: 0 auto; padding: 20px;">
        ${getBrandedHeader(branding)}
        ${html}
        ${getBrandedSignature(branding)}
        ${getBrandedFooter(branding)}
      </body>
    </html>
  `;
}

const handler = async (req: Request): Promise<Response> => {
  // Handle CORS preflight
  if (req.method === "OPTIONS") {
    return new Response(null, { headers: corsHeaders });
  }

  try {
    const apiKey = Deno.env.get("SENDGRID_API_KEY");
    const {
      action,
      to,
      subject,
      message,
      html,
      includeBranding = true,
      attachmentUrl,
      attachmentFilename,
      attachmentMimeType,
      attachments: multipleAttachments,
      branding,
    }: SendGridRequest = await req.json();

    console.log(`SendGrid integration action: ${action}`);

    // Health check - just verify API key is configured
    if (action === "health-check") {
      if (!apiKey) {
        return new Response(
          JSON.stringify({ success: false, error: "SendGrid API key not configured" }),
          { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }
      return new Response(
        JSON.stringify({ success: true, configured: true }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }

    // Test connection - verify API key works
    if (action === "test") {
      if (!apiKey) {
        return new Response(
          JSON.stringify({ success: false, error: "SendGrid API key not configured" }),
          { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }

      // Test API key by fetching user profile
      const testResponse = await fetch("https://api.sendgrid.com/v3/user/profile", {
        method: "GET",
        headers: {
          "Authorization": `Bearer ${apiKey}`,
          "Content-Type": "application/json",
        },
      });

      if (!testResponse.ok) {
        const errorData = await testResponse.json();
        console.error("SendGrid test failed:", errorData);
        return new Response(
          JSON.stringify({ success: false, error: "Invalid API key or connection failed" }),
          { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }

      const userData = await testResponse.json();
      console.log("SendGrid test successful:", userData);
      
      return new Response(
        JSON.stringify({ success: true, message: "SendGrid connection verified" }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }

    // Send email
    if (action === "send-email") {
      if (!apiKey) {
        return new Response(
          JSON.stringify({ success: false, error: "SendGrid API key not configured" }),
          { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }

      if (!to || !subject) {
        return new Response(
          JSON.stringify({ success: false, error: "Missing required fields: to, subject" }),
          { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }

      const emailHtml = html || `<p>${message || ""}</p>`;
      const finalHtml = includeBranding ? wrapWithBranding(emailHtml, branding) : emailHtml;

      let attachmentsList: Array<{ content: string; filename: string; type?: string; disposition?: string }> = [];

      // Helper function to fetch and encode attachment
      const fetchAttachment = async (url: string, filename?: string, mimeType?: string) => {
        console.log(`Fetching attachment from: ${url}`);
        const fileRes = await fetch(url);
        const fileContentType = fileRes.headers.get("content-type") || undefined;
        const fileSizeHeader = fileRes.headers.get("content-length");
        const declaredSize = fileSizeHeader ? Number(fileSizeHeader) : null;

        if (!fileRes.ok) {
          throw new Error(`Failed to fetch attachment: ${fileRes.status}`);
        }

        if (declaredSize !== null && Number.isFinite(declaredSize) && declaredSize > MAX_ATTACHMENT_BYTES) {
          throw new Error(`Attachment too large (${declaredSize} bytes). Max is ${MAX_ATTACHMENT_BYTES} bytes.`);
        }

        const buffer = await fileRes.arrayBuffer();
        const bytes = new Uint8Array(buffer);
        if (bytes.byteLength > MAX_ATTACHMENT_BYTES) {
          throw new Error(`Attachment too large (${bytes.byteLength} bytes). Max is ${MAX_ATTACHMENT_BYTES} bytes.`);
        }

        const resolvedFilename = sanitizeFilename(filename || filenameFromUrl(url) || "document.pdf");
        const resolvedMimeType = mimeType || fileContentType || "application/octet-stream";

        console.log(`Attachment prepared: ${resolvedFilename} (${bytes.byteLength} bytes)`);

        return {
          content: encode(buffer),
          filename: resolvedFilename,
          type: resolvedMimeType,
          disposition: "attachment",
        };
      };

      // Process multiple attachments if provided
      if (multipleAttachments && multipleAttachments.length > 0) {
        try {
          for (const att of multipleAttachments) {
            // If inline base64 content is provided, use it directly (no fetch needed)
            if (att.content) {
              const resolvedFilename = sanitizeFilename(att.filename || "document.pdf");
              const resolvedMimeType = att.mimeType || "application/pdf";
              console.log(`Inline attachment prepared: ${resolvedFilename}`);
              attachmentsList.push({
                content: att.content,
                filename: resolvedFilename,
                type: resolvedMimeType,
                disposition: "attachment",
              });
            } else if (att.url) {
              const attachment = await fetchAttachment(att.url, att.filename, att.mimeType);
              attachmentsList.push(attachment);
            }
          }
        } catch (e: unknown) {
          const msg = e instanceof Error ? e.message : "Unknown error";
          console.error("Attachment handling error:", e);
          return new Response(
            JSON.stringify({ success: false, error: `Attachment error: ${msg}` }),
            { status: 500, headers: { "Content-Type": "application/json", ...corsHeaders } }
          );
        }
      }
      // Legacy single attachment support
      else if (attachmentUrl) {
        try {
          const attachment = await fetchAttachment(attachmentUrl, attachmentFilename, attachmentMimeType);
          attachmentsList.push(attachment);
        } catch (e: unknown) {
          const msg = e instanceof Error ? e.message : "Unknown error";
          console.error("Attachment handling error:", e);
          return new Response(
            JSON.stringify({ success: false, error: `Attachment error: ${msg}` }),
            { status: 500, headers: { "Content-Type": "application/json", ...corsHeaders } }
          );
        }
      }

      // Sender priority: env secret > org branding > fallback.
      // SendGrid REJECTS unverified senders with 403, so the env-configured
      // verified address must win over per-org branding values.
      const envFromEmail = Deno.env.get("SENDGRID_FROM_EMAIL");
      const envFromName = Deno.env.get("SENDGRID_FROM_NAME");
      const fromEmail = envFromEmail || branding?.email || BRAND_EMAIL;
      const fromName = envFromName || branding?.displayName || BRAND_NAME;
      const replyToEmail = !envFromEmail && branding?.email ? branding.email : undefined;

      const emailData: Record<string, unknown> = {
        personalizations: [{ to: [{ email: to }] }],
        from: { email: fromEmail, name: fromName },
        subject: subject,
        content: [
          { type: "text/plain", value: message || subject },
          { type: "text/html", value: finalHtml },
        ],
        ...(attachmentsList.length > 0 ? { attachments: attachmentsList } : {}),
      };
      if (replyToEmail && replyToEmail !== fromEmail) {
        emailData.reply_to = { email: replyToEmail, name: fromName };
      }

      console.log(`Sending email from <${fromEmail}> to <${to}> subject: ${subject}`);

      const sendResponse = await fetch("https://api.sendgrid.com/v3/mail/send", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${apiKey}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify(emailData),
      });

      if (!sendResponse.ok) {
        const errorText = await sendResponse.text();
        let parsed: unknown = null;
        let friendly = errorText;
        try {
          parsed = JSON.parse(errorText);
          const errs = (parsed as { errors?: Array<{ message?: string; field?: string }> })?.errors;
          if (Array.isArray(errs) && errs.length > 0) {
            friendly = errs.map((e) => e.message).filter(Boolean).join("; ");
          }
        } catch { /* keep raw text */ }

        if (sendResponse.status === 403 && /verified sender|verify a sender|sender identity/i.test(friendly)) {
          friendly = `SendGrid rejected the sender "${fromEmail}". Verify this address (or your domain) in SendGrid → Sender Authentication, or set the SENDGRID_FROM_EMAIL secret to a verified sender.`;
        }

        console.error("SendGrid send failed:", { status: sendResponse.status, body: parsed ?? errorText, from: fromEmail });
        // Return 200 so supabase-js exposes the body instead of throwing a
        // generic "non-2xx" error that hides the real reason.
        return new Response(
          JSON.stringify({
            success: false,
            error: friendly,
            status: sendResponse.status,
            details: parsed ?? errorText,
          }),
          { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }

      // SendGrid returns 202 Accepted with no body on success
      const messageId = sendResponse.headers.get("X-Message-Id");
      console.log("Email sent successfully, message ID:", messageId);

      return new Response(
        JSON.stringify({ success: true, messageId: messageId || "sent", from: fromEmail }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }

    // Send a minimal test email to verify end-to-end deliverability.
    if (action === "send-test") {
      if (!apiKey) {
        return new Response(
          JSON.stringify({ success: false, error: "SendGrid API key not configured" }),
          { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }
      if (!to) {
        return new Response(
          JSON.stringify({ success: false, error: "Missing 'to' address" }),
          { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }

      const fromEmail = Deno.env.get("SENDGRID_FROM_EMAIL") || branding?.email || BRAND_EMAIL;
      const fromName = Deno.env.get("SENDGRID_FROM_NAME") || branding?.displayName || BRAND_NAME;
      const testHtml = `<p>This is a test email from <strong>${fromName}</strong> via SendGrid.</p><p>If you received this, your sender (<code>${fromEmail}</code>) and API key are working correctly.</p>`;

      const sendResponse = await fetch("https://api.sendgrid.com/v3/mail/send", {
        method: "POST",
        headers: {
          "Authorization": `Bearer ${apiKey}`,
          "Content-Type": "application/json",
        },
        body: JSON.stringify({
          personalizations: [{ to: [{ email: to }] }],
          from: { email: fromEmail, name: fromName },
          subject: "SendGrid test email — efinsuite",
          content: [
            { type: "text/plain", value: "SendGrid test email. Your configuration is working." },
            { type: "text/html", value: testHtml },
          ],
        }),
      });

      if (!sendResponse.ok) {
        const errorText = await sendResponse.text();
        let friendly = errorText;
        try {
          const parsed = JSON.parse(errorText);
          const errs = parsed?.errors;
          if (Array.isArray(errs) && errs.length > 0) {
            friendly = errs.map((e: { message?: string }) => e.message).filter(Boolean).join("; ");
          }
        } catch { /* ignore */ }
        if (sendResponse.status === 403 && /verified sender|verify a sender|sender identity/i.test(friendly)) {
          friendly = `SendGrid rejected the sender "${fromEmail}". Verify it in SendGrid → Sender Authentication, or set the SENDGRID_FROM_EMAIL secret to a verified address.`;
        }
        console.error("SendGrid test send failed:", { status: sendResponse.status, from: fromEmail, body: errorText });
        return new Response(
          JSON.stringify({ success: false, error: friendly, status: sendResponse.status, from: fromEmail }),
          { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
        );
      }

      const messageId = sendResponse.headers.get("X-Message-Id");
      return new Response(
        JSON.stringify({ success: true, messageId: messageId || "sent", from: fromEmail, to }),
        { status: 200, headers: { "Content-Type": "application/json", ...corsHeaders } }
      );
    }


    return new Response(
      JSON.stringify({ success: false, error: "Unknown action" }),
      { status: 400, headers: { "Content-Type": "application/json", ...corsHeaders } }
    );
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : "Unknown error";
    console.error("SendGrid integration error:", error);
    return new Response(
      JSON.stringify({ success: false, error: errorMessage }),
      { status: 500, headers: { "Content-Type": "application/json", ...corsHeaders } }
    );
  }
};

serve(handler);
