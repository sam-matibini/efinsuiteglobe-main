...
AS $$
  WITH tax_accounts AS (
    -- All GL accounts that look like sales-tax accounts (collected/paid).
    SELECT
      a.id,
      a.code,
      a.name,
  ...
  account_code_match AS (
    -- Best-effort representative tax_code per (account, side).
  ...
  account_code_fallback AS (
    -- If no direct mapping, try matching by account-name keyword to a tax_code with a similar code.
    SELECT
