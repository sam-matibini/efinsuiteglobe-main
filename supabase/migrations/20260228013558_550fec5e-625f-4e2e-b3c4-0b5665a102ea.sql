...
-- Fix reversal netting bug: all three RPCs must include both 'posted' AND 'reversed' entries
-- so that reversed originals and their reversing entries net to zero.

-- 1. Fix calculate_opening_retained_earnings
...
BEGIN
  ...
  -- FIX: Include both 'posted' and 'reversed' for reversal netting
  SELECT MIN(
    CASE 
...
-- 2. Fix calculate_period_net_income
...
-- 3. Fix calculate_retained_earnings_statement
