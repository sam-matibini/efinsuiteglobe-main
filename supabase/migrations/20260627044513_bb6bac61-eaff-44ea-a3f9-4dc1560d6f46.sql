...
BEGIN
  ...
  IF v_retained_earnings_id IS NULL THEN
    ...
    IF EXISTS (SELECT 1 FROM public.accounts WHERE organization_id = p_organization_id AND code = v_new_code) THEN
      v_new_code := '3-00-2' || lpad((floor(random()*900 + 100))::text, 3, '0');
    END IF;
  ...
  -- Closing JE (draft → posted only if there are lines to post)
  ...
  -- Offsetting equity line only when there is a real amount to transfer
  IF v_net_income > 0.001 THEN
    INSERT INTO public.journal_entry_lines (journal_entry_id, account_id, description, debit, credit)
    VALUES (v_journal_entry_id, v_retained_earnings_id,
            format('Transfer FY%s Net Result to %s', p_fiscal_year, v_re_account_name),
            0, v_net_income);
  ELSIF v_net_income < -0.001 THEN
    INSERT INTO public.journal_entry_lines (journal_entry_id, account_id, description, debit, credit)
    VALUES (v_journal_entry_id, v_retained_earnings_id,
  ...
  SELECT
    COUNT(*),
    COALESCE(SUM(COALESCE(base_currency_debit, debit, 0)), 0),
    COALESCE(SUM(COALESCE(base_currency_credit, credit, 0)), 0)
  INTO v_line_count, v_base_debits, v_base_credits
  FROM public.journal_entry_lines
  WHERE journal_entry_id = v_journal_entry_id;

  -- Zero-activity years should be audit-closed without a journal entry.
  IF v_line_count = 0 THEN
    DELETE FROM public.journal_entries WHERE id = v_journal_entry_id;
    v_journal_entry_id := NULL;
  ELSE
    IF v_line_count < 2 THEN
      RAISE EXCEPTION 'Fiscal year close generated only % journal line. Review temporary account balances before closing FY%.', v_line_count, p_fiscal_year;
    END IF;

    IF ABS(v_base_debits - v_base_credits) > 0.02 THEN
      RAISE EXCEPTION 'Fiscal year close generated an unbalanced entry. Base debits (%) must equal base credits (%).', v_base_debits, v_base_credits;
    END IF;
  ...
  INSERT INTO public.fiscal_year_closes (
    ...
    v_net_income, v_retained_earnings_id, v_journal_entry_id, v_user_id,
    COALESCE(p_notes, CASE WHEN v_journal_entry_id IS NULL THEN 'Closed with no temporary account balances to transfer.' ELSE NULL END)
  )
  ...
  RETURN QUERY SELECT
    TRUE::BOOLEAN,
    CASE
      WHEN v_journal_entry_id IS NULL THEN
        format('Successfully closed fiscal year %s. No closing journal entry was required because net income was zero.', p_fiscal_year)::TEXT
      ELSE
...
'Closes a fiscal year by creating closing entries to transfer net income to the equity rollup account (Retained Earnings for for-profit, Unrestricted Net Assets for NPO). Zero-activity years are marked closed without an invalid journal entry. GAAP/IFRS/ASPE/ASNPO compliant.';