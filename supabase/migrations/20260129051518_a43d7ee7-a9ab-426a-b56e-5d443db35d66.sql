...
-- This ensures Opening RE (Year N) = RE Account Activity (prior years) + Cumulative Net Income (unclosed prior years)
-- Matches the logic in get_retained_earnings_rollforward_series for consistency across all organizations
...
BEGIN
  -- Determine first year of operations
  ...
  -- This is CRITICAL for organizations that haven't run year-end closing
...
-- Also update get_equity_breakdown to use the corrected function
...
BEGIN
  ...
  SELECT COALESCE(SUM(
    COALESCE(a.opening_balance, 0) + COALESCE(
      (SELECT SUM(CASE WHEN a.normal_balance = 'credit' THEN jel2.credit - jel2.debit ELSE jel2.debit - jel2.credit END)
       ...
       JOIN public.journal_entries je2 ON jel2.journal_entry_id = je2.id
  ...
  -- Opening Retained Earnings (uses fixed function with unclosed NI)
