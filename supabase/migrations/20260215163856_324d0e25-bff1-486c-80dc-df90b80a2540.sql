...
DECLARE
  ...
  v_closing NUMERIC;
BEGIN
  -- Get fiscal year end month for the organization
  ...
  -- Delete existing movements for this org/year
  ...
  IF v_re_account IS NOT NULL THEN
    ...
    IF v_distributions <> 0 THEN
      INSERT INTO equity_movements (organization_id, fiscal_year, equity_account_id, movement_type, amount)
      VALUES (p_organization_id, p_fiscal_year, v_re_account.id, 'DISTRIBUTIONS', v_distributions);
    END IF;
  ...
  -- ============================================================
  -- COMMON STOCK (all accounts with equity_category = 'COMMON_STOCK')
  -- ============================================================
  FOR v_sc_account IN
    ...
    WHERE organization_id = p_organization_id
      ...
      AND is_header = false
  LOOP
    -- Opening: initial balance + prior-year GL activity
    ...
    INSERT INTO equity_movements (organization_id, fiscal_year, equity_account_id, movement_type, amount)
    VALUES
      (p_organization_id, p_fiscal_year, v_sc_account.id, 'OPENING_BALANCE', v_opening);

    IF v_contributions <> 0 THEN
      INSERT INTO equity_movements (organization_id, fiscal_year, equity_account_id, movement_type, amount)
      VALUES (p_organization_id, p_fiscal_year, v_sc_account.id, 'CONTRIBUTIONS', v_contributions);
    END IF;
