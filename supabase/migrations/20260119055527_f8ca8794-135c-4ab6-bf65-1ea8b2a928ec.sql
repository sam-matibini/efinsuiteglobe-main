-- Fix close_fiscal_year: insert as 'draft' first, then update to 'posted' after all lines are added
CREATE OR REPLACE FUNCTION public.close_fiscal_year(p_organization_id uuid, p_fiscal_year integer, p_fiscal_year_start date, p_fiscal_year_end date, p_notes text DEFAULT NULL::text)
 RETURNS TABLE(success boolean, message text, net_income numeric, closing_entry_id uuid, fiscal_year_close_id uuid)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Create closing journal entry as DRAFT first (triggers won't block unbalanced drafts)
  INSERT INTO public.journal_entries (
    ...
    'draft',  -- Start as draft to avoid trigger validation until all lines added
    'adjustment',
    v_user_id
  )
